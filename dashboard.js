(() => {
  ...
  function eventCard(event) {
    ...
    const date = document.createElement('p'); date.textContent = `Created ${new Date(event.createdAt).toLocaleDateString()}`;
    info.append(title, date);
    const actions = document.createElement('div'); actions.className = 'actions';
    const manage = document.createElement('button'); manage.className = 'action'; manage.type = 'button'; manage.textContent = 'Manage';
    manage.addEventListener('click', () => openManager(event.id));
    const rename = document.createElement('button'); rename.className = 'action'; rename.type = 'button'; rename.textContent = 'Rename';
    ...
    actions.append(manage, rename, remove); card.append(info, actions); return card;
  }
  ...
  function renderManager(event) {
    ...
    event.nominees.forEach(nominee => { const category = event.categories.find(item => item.id === nominee.categoryId); const detail = `${category ? category.name : 'Uncategorised'} · Code: ${nominee.code || 'Pending'}`; nomineesElement.append(listItem(nominee.name, detail, async () => { if (!window.confirm(`Delete “${nominee.name}”?`)) return; try { await request(`/api/events/${selectedEventId}/nominees/${nominee.id}`, { method: 'DELETE' }); await openManager(selectedEventId); } catch (error) { showNotice(error.message, true); } })); });
  ...
  async function loadWithdrawals() {
    ...
    document.querySelector('#availableWithdrawal').textContent = `GHS ${Number(withdrawal.available || 0).toFixed(2)} available`;
  ...
  async function initialise() {
    try { const { user } = await request('/api/auth/me'); accountName.textContent = user.name; profileForm.elements.name.value = user.name; profileForm.elements.email.value = user.email; await Promise.all([loadEvents(), loadStats(), loadWithdrawals()]); }
  ...
  profileForm.addEventListener('submit', async event => { event.preventDefault(); if (!profileForm.reportValidity()) return; try { const { user } = await request('/api/auth/profile', { method: 'PATCH', body: JSON.stringify({ name: profileForm.elements.name.value, email: profileForm.elements.email.value }) }); accountName.textContent = user.name; profileForm.elements.name.value = user.name; profileForm.elements.email.value = user.email; localStorage.setItem('voteusUser', JSON.stringify(user)); showProfileNotice('Profile updated.'); } catch (error) { showProfileNotice(error.message, true); } });
